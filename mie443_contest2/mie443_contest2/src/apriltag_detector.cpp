...
bool AprilTagDetector::isTagVisible(int tag_id, int timeout_ms) {
    ...
    try {
        ...
        // Check if transform is recent (within 2 seconds to account for network latency)
        auto transform_age = node_->get_clock()->now() - transform.header.stamp;
        if (transform_age.seconds() > 2.0) {
            RCLCPP_DEBUG(node_->get_logger(), "Tag %d transform is stale (%.2fs old)", tag_id, transform_age.seconds());
            return false;  // Transform is stale - tag no longer visible
        }
        
        RCLCPP_INFO(node_->get_logger(), "Tag %d is visible (transform age: %.2fs)", tag_id, transform_age.seconds());
        return true;
    } catch (const tf2::TransformException& ex) {
        // Transform not available - tag never seen
        RCLCPP_DEBUG(node_->get_logger(), "Tag %d not in TF tree: %s", tag_id, ex.what());
        return false;
    }
}
