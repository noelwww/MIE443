...
YoloInterface::YoloInterface(std::shared_ptr<rclcpp::Node> node)
    : node_(node), latest_confidence_(0.0), has_detection_(false) {
...
bool YoloInterface::captureAndDetect(CameraSource camera, bool save_image) {
    ...
    // Wait for the future directly — the node is already being spun by
    // the MultiThreadedExecutor in main().  Using spin_until_future_complete()
    // would crash with "already added to an executor".
    if (capture_future.wait_for(5s) != std::future_status::ready) {
    ...
    // Wait directly — same reason as above (node already on an executor).
    if (detect_future.wait_for(5s) != std::future_status::ready) {
...
std::string YoloInterface::getObjectName(CameraSource camera, bool save_image) {
    // Pass the boolean down to the next function
