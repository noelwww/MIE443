#pragma once

#include <memory>
#include <thread>
#include <rclcpp/rclcpp.hpp>
#include <moveit/move_group_interface/move_group_interface.hpp>
#include <geometry_msgs/msg/pose.hpp>


class ArmController {
public:
	ArmController(std::shared_ptr<rclcpp::Node> node);
	~ArmController();

	bool moveToCartesianPose(double x, double y, double z, 
	                         double roll, double pitch, double yaw);

	bool moveToCartesianPose(double x, double y, double z,
	                         double qx, double qy, double qz, double qw);

	// Move arm by specifying joint angles directly (bypasses IK entirely)
	// joints = {joint1, joint2, joint3, joint4, joint5} in radians
	bool moveToJointTarget(const std::vector<double>& joint_values);

	// Move arm to a named pose defined in the SRDF (e.g. "home")
	bool moveToNamedTarget(const std::string& name);

	bool moveGripper(double position);

	bool openGripper();

	bool closeGripper();

	// Read current end-effector pose (for calibrating arm pose parameters)
	geometry_msgs::msg::PoseStamped getCurrentPose();

	// Read current joint values (for debugging)
	std::vector<double> getCurrentJointValues();

	// Get the planning frame name
	std::string getPlanningFrame();

	// Get the end-effector link name
	std::string getEndEffectorLink();

private:
	std::shared_ptr<rclcpp::Node> node_;
	std::shared_ptr<rclcpp::Node> moveit_node_;
	std::shared_ptr<rclcpp::executors::SingleThreadedExecutor> moveit_executor_;
	std::thread moveit_spinner_;
	std::shared_ptr<moveit::planning_interface::MoveGroupInterface> arm_group_;
	std::shared_ptr<moveit::planning_interface::MoveGroupInterface> gripper_group_;
};

